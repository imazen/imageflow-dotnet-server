...
public readonly struct UrlHostString : IEquatable<UrlHostString>
{
    // Allowed Characters:
    // A-Z, a-z, 0-9, .,
    // -, %, [, ], :
    ...
    public UrlHostString(string value)
    {
        _value = value;
    }
    ...
    public UrlHostString(string host, int port)
    {
        ArgumentNullThrowHelper.ThrowIfNull(host);

        if (port <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(port), port, "The port must be greater than zero.");
        }
    ...
    /// </summary>
    /// <returns>The <see cref="UrlHostString"/> value formated for use in a URI or HTTP header.</returns>
    ...
    /// </summary>
    /// <param name="uriComponent">The URI component string to create a <see cref="UrlHostString"/> from.</param>
    /// <returns>The <see cref="UrlHostString"/> that was created.</returns>
    public static UrlHostString FromUriComponent(string uriComponent)
    {
...
#else
                    uriComponent = string.Concat(s_idnMapping.GetUnicode(uriComponent, 0, index), port.ToString());
#endif
                }
                else
                {
        ...
        return new UrlHostString(uriComponent);
    }
    ...
    /// </summary>
    /// <param name="uri">The <see cref="Uri"/> to create a <see cref="UrlHostString"/> from.</param>
    /// <returns>The <see cref="UrlHostString"/> that was created.</returns>
    public static UrlHostString FromUriComponent(Uri uri)
    {
        ArgumentNullThrowHelper.ThrowIfNull(uri);

        return new UrlHostString(uri.GetComponents(
    ...
    public static bool MatchesAny(StringSegment value, IList<StringSegment> patterns)
    {
        ...
        ArgumentNullThrowHelper.ThrowIfNull(patterns);
    ...
    /// </summary>
    /// <param name="other">The <see cref="UrlHostString"/> to compare against.</param>
    /// <returns><see langword="true" /> if they have the same value.</returns>
    public bool Equals(UrlHostString other)
    {
    ...
    public override bool Equals(object? obj)
    {
        ...
        return obj is UrlHostString && Equals((UrlHostString)obj);
    }
    ...
    /// <returns><see langword="true" /> if both <see cref="UrlHostString"/>'s have the same value.</returns>
    public static bool operator ==(UrlHostString left, UrlHostString right)
    {
    ...
    /// <param name="left">The left parameter.</param>
    /// <param name="right">The right parameter.</param>
    /// <returns><see langword="true" /> if both <see cref="UrlHostString"/>'s values are not equal.</returns>
    public static bool operator !=(UrlHostString left, UrlHostString right)
    {
        return !left.Equals(right);
    }
