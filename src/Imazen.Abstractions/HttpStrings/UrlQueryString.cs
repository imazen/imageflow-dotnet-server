...
public readonly struct UrlQueryString : IEquatable<UrlQueryString>
{
    /// <summary>
    /// Represents the empty query string. This field is read-only.
    /// </summary>
    public static readonly UrlQueryString Empty = new UrlQueryString(string.Empty);
    ...
    public UrlQueryString(string? value)
    {
    ...
    public Dictionary<string, StringValues>? ParseNullable()
    {
        return QueryHelpers.ParseNullableQuery(Value);
    }
    
    public Dictionary<string, StringValues> Parse()
    {
        return QueryHelpers.ParseQuery(Value);
    }
    ...
    public static UrlQueryString FromUriComponent(string uriComponent)
    {
        if (string.IsNullOrEmpty(uriComponent))
        {
            return new UrlQueryString(string.Empty);
        }
        return new UrlQueryString(uriComponent);
    }
    ...
    /// <returns>The resulting QueryString</returns>
    public static UrlQueryString FromUriComponent(Uri uri)
    {
        ...
        return new UrlQueryString(queryValue);
    }
    ...
    /// <returns>The resulting QueryString</returns>
    public static UrlQueryString Create(string name, string value)
    {
        ...
        return new UrlQueryString($"?{UrlEncoder.Default.Encode(name)}={value}");
    }
    ...
    /// <returns>The resulting QueryString</returns>
    public static UrlQueryString Create(IEnumerable<KeyValuePair<string, string?>> parameters)
    {
        ...
        return new UrlQueryString(builder.ToString());
    }
    ...
    /// <param name="parameters"></param>
    /// <returns>The resulting QueryString</returns>
    public static UrlQueryString Create(IEnumerable<KeyValuePair<string, StringValues>> parameters)
    {
        var builder = new StringBuilder();
        var first = true;
    ...
    /// </summary>
    /// <param name="other">The <see cref="UrlQueryString"/> to concatenate.</param>
    /// <returns>The concatenated <see cref="UrlQueryString"/>.</returns>
    public UrlQueryString Add(UrlQueryString other)
    {
        ...
        // NOTE: had to add .ToString() for compatibility with .NET Framework
#if NETSTANDARD2_1_OR_GREATER
        return new QueryString(string.Concat(Value, "&", other.Value.AsSpan(1)));
#else
        return new UrlQueryString(string.Concat(Value, "&", other.Value[1..]));
#endif
    }
    ...
    /// </summary>
    /// <param name="other">The <see cref="UrlQueryString"/> to compare.</param>
    /// <returns><see langword="true"/> if the query strings are equal.</returns>
    public bool Equals(UrlQueryString other)
    {
    ...
    public override bool Equals(object? obj)
    {
        ...
        return obj is UrlQueryString && Equals((UrlQueryString)obj);
    }
    ...
    /// </summary>
    /// <param name="left">A <see cref="UrlQueryString"/> instance.</param>
    /// <param name="right">A <see cref="UrlQueryString"/> instance.</param>
    /// <returns><see langword="true" /> if the query strings are equal.</returns>
    public static bool operator ==(UrlQueryString left, UrlQueryString right)
    {
    ...
    /// <param name="right">A <see cref="UrlQueryString"/> instance.</param>
    /// <returns>The concatenated <see cref="UrlQueryString"/>.</returns>
    public static UrlQueryString operator +(UrlQueryString left, UrlQueryString right)
    {
